...
int main()
{
    ...
    //BaseNode *L =  nn.negative<long double>((nn.log<long double>(G)));

    // intermidiate loss function
    BaseNode *L = nn.negative<long double>(nn.add<long double>(nn.matmultiply<long double>(nn.variable<long double>(Y), nn.log<long double>(G)), nn.matmultiply<long double>(nn.add<long double>(nn.variable<long double>(one), nn.negative<long double>(nn.variable<long double>(Y))), nn.log<long double>(nn.add<long double>(nn.variable<long double>(one), nn.negative<long double>(G))))));

    // Create gradient descent optimization
    auto opt = GradientDescentOptimizer(0.0).minimize<matxxf>(L);
